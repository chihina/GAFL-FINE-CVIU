...
def prep_images_c3d_ucf101(images):
    """
    preprocess images
    Args:
        images: pytorch tensor
    ...
    images = images.reshape(B*T*N, C, H, W)
    images = normalizer(images)
    images = images.reshape(B, T, N, C, H, W)

    return images

def prep_images_unipose(images):
    """
    preprocess images
    Args:
        images: pytorch tensor
    """
    images = torch.sub(images, 128.0)
    images = torch.div(images, 256.0)

    return images

# def prep_images_c3d(images):
#     """
#     preprocess images
#     Args:
#         images: pytorch tensor
...
def prep_images_3dresnet(images):
    """
    preprocess images
    Args:
        images: pytorch tensor
    ...
    resizer = transforms.Resize((128, 171))

    B, T, _, N, C, H, W = images.shape
    images = images.reshape((B*T*3*N, C, H, W))
    images = images.div(255.0)
    images = normalizer(images)
    images = resizer(images)
    images = images.reshape((B, T, 3, N, C, 128, 171))
...
def show_epoch_info(phase, log_path, info):
    ...
    print_log(log_path, 'Individual Action Accuracy: %.2f%%, Loss: %.5f, Using %.1f seconds'%(
                info['actions_acc'], info['loss'], info['time']))

    if 'actions_conf' in info.keys():
        print_log(log_path, info['actions_conf'])
    if 'actions_MPCA' in info.keys():
        print_log(log_path, 'Actions MPCA:{:.2f}%'.format(info['actions_MPCA']))
...
def gen_ja_points(ja_bboxes):
    ja_bboxes_xmin, ja_bboxes_ymin, ja_bboxes_xmax, ja_bboxes_ymax = ja_bboxes[:, 0], ja_bboxes[:, 1], ja_bboxes[:, 2], ja_bboxes[:, 3]
    ja_bboxes_xmid = (ja_bboxes_xmin + ja_bboxes_xmax) / 2
    ja_bboxes_ymid = (ja_bboxes_ymin + ja_bboxes_ymax) / 2
    ja_points = np.vstack([ja_bboxes_xmid, ja_bboxes_ymid]).T
