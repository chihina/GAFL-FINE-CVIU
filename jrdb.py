...
def read_excel(excel_dict_path):
    ...
    for path in excel_path_list:
        excel_path = os.path.join(excel_dict_path, path)
        # read excel file
...
def jrdb_read_dataset_new(path, seqs, num_actions, num_activities, num_social_activities):
    action_path = os.path.join(path, 'annotations', 'gt_action.txt')
    activity_path = os.path.join(path, 'annotations', 'gt_activity.txt')
    group_path = os.path.join(path, 'annotations', 'gt_group.txt')
    excel_path = os.path.join(path, 'annotations', 'annotated_excel')
    ...
    for seq_id in social_activity_dict.keys():
        for frame_id in social_activity_dict[seq_id].keys():
            for person_id in social_activity_dict[seq_id][frame_id].keys():
                ...
                # if seq_id in global_activity_dict and frame_id in global_activity_dict[seq_id]:
                #     group_labels = global_activity_dict[seq_id][frame_id]['global_labels']
                # else:
                #     print(f"[WARNING] global_labels missing for seq_id={seq_id}, frame_id={frame_id}")
                #     group_labels = []
                    ...
                    if frame_id not in data_all[seq_id]:
                        data_all[seq_id][frame_id] = {
                            ...
                            'group_activity_all': one_hot(global_activity_dict[seq_id][15]['global_labels'],
                                                        num_activities),
                            # 'group_activity_all': one_hot(global_activity_dict[seq_id][frame_id]['global_labels'],
                            #                                 num_activities),
                            'social_activity_all': [],
                            'actions_all': [],
                        }
...
class JRDB_Dataset(data.Dataset):
    ...
    def __init__(self, num_actions, num_activities, num_social_activities, anns, frames, images_path, image_size,
                 feature_size, args, num_boxes=13, num_frame=10,
        ...
        self.person_size = args.person_size

        self.num_boxes = num_boxes
        self.num_frame = num_frame
        self.num_total_frame = 15

        self.is_training = is_training
        self.is_finetune = is_finetune
        self.random_sampling = args.random_sampling
        self.use_flow_numpy = False
    ...
    def get_num_boxes_max(self):
        ...
        for sid, src_fid in self.frames:
            bboxes = self.anns[sid][src_fid]['bboxes']
            max_num_boxes = max(max_num_boxes, len(bboxes))
    ...
    def load_samples_sequence(self, select_frames):
        ...
        for i, (sid, src_fid, fid) in enumerate(select_frames):
            ...
            if os.path.exists(this_image_path):
                ...
                img_people = Image.open(this_image_path)
            else:
                img = Image.open(self.images_path + '/' + seq_names[int(sid)] + '/' + str(src_fid).zfill(6) + ".jpg")
                img_people = Image.open(self.images_path + '/' + seq_names[int(sid)] + '/' + str(src_fid).zfill(6) + ".jpg")
            IW, IH = img.size
            ...
            if self.use_flow_numpy:
                flow_dir = self.images_path.replace('videos', 'flow_numpy_sub_med_8x63')
                this_flow_path = flow_dir + '/' + seq_names[int(sid)] + '/' + str(fid).zfill(6) + "_flow.npy"
                if os.path.exists(this_flow_path):
                    flow = np.load(this_flow_path)
                else:
                    flow = np.load(flow_dir + '/' + seq_names[int(sid)] + '/' + str(src_fid).zfill(6) + "_flow.npy")
            ...
            for box in self.anns[sid][src_fid]['bboxes']:
                ...
                temp_images_person.append(np.array(img_person))
            ...
            while len(temp_boxes) != self.num_boxes:
                ...
                temp_sid.append(-1)
                temp_table.append({})
                temp_images_person.append(temp_images_person[-1])
            ...
            activities_all.append(self.anns[sid][src_fid]['group_activity_all'])
        ...
        activities = np.array(activities, dtype=np.int32).reshape(-1)
        activities_all = np.array(activities_all, dtype=np.int32).reshape((-1, self.num_activities))
        ...
        data['activities_all_in'] = activities_all
