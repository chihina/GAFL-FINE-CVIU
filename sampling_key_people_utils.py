...
def sampling_key_people(inp_dic):
    ...
    key_peopleness = torch.zeros(len(training_set), device=device)
    with torch.no_grad():
        for batch_idx, batch_data in enumerate(tqdm(all_loader)):
            ...
            masking_trials = 5
            key_peopleness_trial = torch.zeros(batch_size, masking_trials)
            ...
            for trial_idx in range(masking_trials):
                ...
                # compute the euclidean distance between the original and perturbed GAF
                gaf_diff = gaf_original - gaf_purturbation
                gaf_diif_dist = gaf_diff.norm(dim=-1)
                key_peopleness_trial[:, trial_idx] = gaf_diif_dist

            key_peopleness[start_batch_idx:finish_batch_idx] = key_peopleness_trial.mean(dim=-1)
        
    return key_peopleness